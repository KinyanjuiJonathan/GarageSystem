...
export default function GaradgeUpdateForm(props) {
  const {
    id: idProp,
    garadge,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = garadgeRecord
      ? { ...initialValues, ...garadgeRecord }
      : initialValues;
  ...
  const [garadgeRecord, setGaradgeRecord] = React.useState(garadge);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Garadge, idProp) : garadge;
      setGaradgeRecord(record);
    };
    queryData();
  }, [idProp, garadge]);
  React.useEffect(resetStateValues, [garadgeRecord]);
  const validations = {
    name: [{ type: "Required" }],
    image: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Garadge.copyOf(garadgeRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "GaradgeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
